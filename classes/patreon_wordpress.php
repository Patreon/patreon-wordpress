...
class Patreon_Wordpress
{
    ...
    public static function setup_wizard()
    {
        ...
        if (isset($_REQUEST['setup_stage']) and 'reconnect_0' == $_REQUEST['setup_stage']) {
            $requirements_check = Patreon_Compatibility::check_requirements();
            $requirement_notices = '';
    ...
    public static function deactivate()
    {
        ...
        remove_action('generate_rewrite_rules', ['Patreon_Routing', 'add_rewrite_rules']);
    ...
    public static function populate_patreon_level_select_from_ajax()
    {
        ...
        if (!isset($_REQUEST['action']) or 'patreon_wordpress_populate_patreon_level_select' != $_REQUEST['action']) {
            return;
        }
    ...
    public static function make_tiers_select($post = false, $args = [])
    {
        if (!$post) {
            global $post;
        }
        ...
        if (isset($creator_tiers['included']) and is_array($creator_tiers['included'])) {
            ...
            foreach ($creator_tiers['included'] as $key => $value) {
                ...
                if (
                    ...
                    or ('reward' != $creator_tiers['included'][$key]['type'] and 'tier' != $creator_tiers['included'][$key]['type'])
                ) {
                    continue;
                }
    ...
    public static function update_creator_tiers_from_api()
    {
        ...
        if (isset($creator_info) and isset($creator_info['included']) and is_array($creator_info['included']) and isset($creator_info['included'][1]['type']) and 'reward' == $creator_info['included'][1]['type']) {
    ...
    public static function order_independent_actions_to_run_on_init_start()
    {
        ...
        if (isset($_REQUEST['patreon_wordpress_action']) and 'disconnect_site_from_patreon' == $_REQUEST['patreon_wordpress_action'] and is_admin() and current_user_can('manage_options')) {
            ...
            if ($creator_access_token and $client_id) {
                ...
                if (isset($client_result['response']['code']) and '204' == $client_result['response']['code']) {
        ...
        if (isset($_REQUEST['patreon_wordpress_action']) and 'disconnect_site_from_patreon_for_reconnection' == $_REQUEST['patreon_wordpress_action'] and is_admin() and current_user_can('manage_options')) {
    ...
    public function make_user_select($selected_user = 1)
    {
        ...
        $args = [
            'role__in' => ['Super Admin', 'Administrator', 'Editor', 'Author', 'Contributor'],
    ...
    public function download_insert_media($url, $filename)
    {
        ...
        if (!is_wp_error($temp_file)) {
            ...
            $file_array = [ // array to mimic $_FILES
    ...
    public function check_post_sync_webhook()
    {
        ...
        if (false !== strpos($_SERVER['REQUEST_URI'], 'patreon-webhook')) {
            return;
        }

        if ('no' == get_option('patreon-sync-posts', 'no')) {
            // Checks if post sync is enabled and posts webhook in case it is not posted
            $existing_hook = get_option('patreon-post-sync-webhook', []);
        ...
        if (is_array($webhook_added) and isset($webhook_added['data']['type']) and 'webhook' == $webhook_added['data']['type']) {
            // Save webhook info
