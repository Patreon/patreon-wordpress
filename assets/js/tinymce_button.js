...
tinymce.PluginManager.add( 'tinymce_patreon_wordpress_patron_only_image_button_plugin', function( editor ) {
	...
	editor.addButton( 'wp_img_patreon', {
		tooltip: 'Patron Only? ', // trailing space is needed, used for context
		icon: 'dashicon dashicons-admin-network',
		onclick: function() {
			
		//console.log(editor.selection.getNode());
		//console.log(editor.selection.getNode().src);

		var image_classes = editor.selection.getNode().className.split(' ');
